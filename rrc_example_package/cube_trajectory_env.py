...
class BaseCubeTrajectoryEnv(gym.GoalEnv):
    ...
    def _create_raw_observation(self, t, action, obs_type='default'):
        robot_observation = self.platform.get_robot_observation(t)
        camera_observation = self.platform.get_camera_observation(t)
        object_observation = camera_observation.filtered_object_pose

        active_goal = task.get_active_goal(self.info["trajectory"], t)
        ...
        observation = {
            ...
            "achieved_goal": move_cube.Pose(position=object_observation.position,orientation=object_observation.orientation),
        }
...
class SimtoRealEnv(BaseCubeTrajectoryEnv):
    ...
    def __init__(
        ...
        env_type='sim', obs_type='default', env_wrapped=False, increase_fps=False, disable_arm3=False,reward_type ='1'
    ):
        """Initialize.
    ...
    def step(self, action, initial=False):
        ...
        for _ in range(num_steps):
            ...
            observation = self._create_raw_observation(
        ...
        reward = self.compute_reward(observation['achieved_goal'], self._active_goal, self.reward_type)
        self.info["pos_is_success"] = self.compute_sparse_reward(observation['achieved_goal'], self._active_goal, 'pos_success') == 0
        self.info["ori_is_success"] = self.compute_sparse_reward(observation['achieved_goal'], self._active_goal, 'ori_success') == 0
        self.info["is_success"] = self.compute_sparse_reward(observation['achieved_goal'], self._active_goal, 'success') == 0
